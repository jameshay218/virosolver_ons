...
sds_exp <- sds_seir <- sds_gp <- c("beta"=0.25,"obs_sd"=0.5,"viral_peak"=2,
                                   "wane_rate2"=1,"t_switch"=2,"level_switch"=1,
                                   "prob_detect"=0.03,
...
prior_func_seeirr <- function(pars){
  ...
  p4 <- dlnorm(pars["infectious"],log(8),sds["infectious"],log=TRUE)
  p5 <- dlnorm(pars["recovery"],log(7), sds["recovery"], log=TRUE)
